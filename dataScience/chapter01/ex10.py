...
df = pd.read_csv("c:/source/pythonsource/dataScience/data/allowance.csv", encoding="utf-8-sig")
print("=== 원본 용돈지급 데이터 ===")
print(df)
...
# 3. lambda로 활용
df["짠돌이여부"]=df["월용돈"].apply(lambda x: "짠돌이" if x<20000 else "짠돌이 아님")
print("\n=== lambda로 간단 조건 처리 ===")
print(df)
print(df[["이름","월용돈","짠돌이여부"]])

# 4. 여러 컬럼을 동시에 참고해야 할 때 asis=1
df["연간총액"]=df.apply(lambda row:row["월용돈"]*12, axis=1)
print("\n=== lambda로 행전체를 활용할 때 ===")
print(df)
print(df[["이름","월용돈","연간총액"]])

# 5. 등급별 인원수 확인, 부자 등급만 정렬해서 출력
print("\n=== 등급별 인원수(groupby) ===")
print(df["등급"].value_counts())

rich_students=df[df["등급"]=="대인배"].sort_values("월용돈", ascending=False)
print("\n=== '대인배' 등급 학생(용돈이 많은 순서) ===")
