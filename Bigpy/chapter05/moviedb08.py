...
# 영화 인기순위 10개 가져오기
def get_popular_movies(count=10):
...
def main():
    ...
    for i, movie in enumerate(popular_movies, 1):
        ...
        info = {
            ...
            "줄거리": (
                detail["overview"][:80] + "..."
                if len(detail["overview"]) > 80
                else detail["overview"]
            ),
            "포스터": IMAGE_BASE_URL + detail["poster_path"] if detail["poster_path"] else None    # poster_path가 없으면 None으로 표시         
        }

        print(f"{i}위 | {info['제목']} | 평점 {info['평점']} | 개봉 {info['개봉일']}")
        results.append(info)

    with open("popular_movies_top10.json", "w", encoding="utf-8") as f:
        json.dump(results, f, ensure_ascii=False, indent=2)

    print("\n저장 완료: popular_movies_top10.json")
